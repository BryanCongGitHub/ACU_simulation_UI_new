...
class DummyComm:
    def __init__(self):
        self.on_receive=None; self.on_error=None; self.on_status=None
        self.started=False
    def update_config(self, **cfg): pass
    def setup(self): return True
    def start_receive_loop(self): self.started=True
...
def run_send_smoke(app):
    ...
    comm=DummyComm(); parse=ParseController(); state=ControlState()
    device=Device(DeviceConfig(name="ACU", ip="127.0.0.1", send_port=40000, receive_port=40001, category="ACU"))
    frame=FrameBuilder(state, device); bus=ViewEventBus()
    win=ACUSimulator(comm=comm, parse_controller=parse, control_state=state, acu_device=device, frame_builder=frame, view_bus=bus)
    print("SMOKE: ACUSimulator created")
    send_count={'n':0}; bus.waveform_send.connect(lambda *_: send_count.__setitem__('n', send_count['n']+1))
    print("SMOKE: starting communication")
    win.start_communication()
    print("SMOKE: started; calling send_periodic_data twice")
    win.send_periodic_data(); win.send_periodic_data()
    print("SMOKE: calling stop_communication")
    win.stop_communication()
    parse_thr=getattr(win,'parse_worker_thread',None); fmt_thr=getattr(win,'format_worker_thread',None)
    ok = send_count['n']>=2 and (parse_thr is None or not parse_thr.is_alive()) and (fmt_thr is None or not fmt_thr.is_alive())
...
def run_recv_smoke(app):
    ...
    recv_count={'n':0}; bus.waveform_receive.connect(lambda *_: recv_count.__setitem__('n', recv_count['n']+1))
    print("SMOKE: starting communication (recv)")
    win.start_communication()
    fake=bytearray(16); fake[0:2]=(0x55AA).to_bytes(2,'big'); fake[2]=0x42
    print("SMOKE: injecting fake data")
    win.on_data_received_comm(bytes(fake), ("127.0.0.1", 49999)); win.stop_communication()
    parse_thr=getattr(win,'parse_worker_thread',None); fmt_thr=getattr(win,'format_worker_thread',None)
    ok = recv_count['n']>=1 and (parse_thr is None or not parse_thr.is_alive()) and (fmt_thr is None or not fmt_thr.is_alive())
