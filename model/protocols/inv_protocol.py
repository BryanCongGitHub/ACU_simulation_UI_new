...
class InvLikeProtocol(BaseProtocol):
    ...
    def parse_receive_frame(self, data: bytes) -> Dict[str, Any]:
        ...
        fault_info = []
        if self._cat != "BCC":
            if fault_byte1 & 0x01: fault_info.append("模块A相管保护")
            if fault_byte1 & 0x02: fault_info.append("模块B相管保护")
            if fault_byte1 & 0x04: fault_info.append("模块C相管保护")
            if fault_byte1 & 0x08: fault_info.append("模块过热")
            if fault_byte1 & 0x10: fault_info.append("模块输出短路过流")
            if fault_byte1 & 0x20: fault_info.append("模块输出三相不平衡")
            if fault_byte1 & 0x40: fault_info.append("输出过流")
            if fault_byte1 & 0x80: fault_info.append("IPM电流异常")
            if fault_byte2 & 0x01: fault_info.append("模块输出1.5倍过载")
            if fault_byte2 & 0x02: fault_info.append("模块输出1.2倍过载")
            if fault_byte2 & 0x04: fault_info.append("Fuse熔断")
            if fault_byte2 & 0x80: fault_info.append("生命信号故障")
        else:  # BCC 特殊
            if fault_byte1 & 0x01: fault_info.append("原边过流")
            if fault_byte1 & 0x02: fault_info.append("输出过压")
            if fault_byte1 & 0x04: fault_info.append("输出欠压")
            if fault_byte1 & 0x08: fault_info.append("输出过流")
            if fault_byte1 & 0x10: fault_info.append("蓄电池充电过流")
            if fault_byte1 & 0x20: fault_info.append("模块管保护")
            if fault_byte1 & 0x40: fault_info.append("输入熔断器故障")
            if fault_byte1 & 0x80: fault_info.append("蓄电池温度过高")
            if fault_byte2 & 0x01: fault_info.append("蓄电池温度传感器故障")
            if fault_byte2 & 0x02: fault_info.append("模块温度传感器故障")
            if fault_byte2 & 0x04: fault_info.append("模块过温")
