...
class DataBuffer:
    ...
    def add_data_point(self, signal_id, value, timestamp=None):
        ...
        logger.debug(f"添加单个数据点: {signal_id} = {value}, 时间戳数: {len(self.timestamps)}")
    ...
    def add_data_points(self, signal_values, timestamp=None):
        ...
        # 只添加一个时间戳
        self.timestamps.append(timestamp)
        logger.debug(f"添加批量数据点: {list(signal_values.keys())}, 时间戳: {timestamp:.3f}")
        ...
        for signal_id, value in signal_values.items():
            ...
            if signal_id not in self.signal_order:
                ...
                for _ in range(len(self.timestamps) - 1):
                    self.data[signal_id].append(value)
                logger.debug(f"新信号 {signal_id} 已添加, 填充 {len(self.timestamps)-1} 个历史点")
        ...
        self._sync_data_length()
        logger.debug(f"批量添加完成: 时间戳数={len(self.timestamps)}, 信号数={len(self.signal_order)}")
    ...
    def _sync_data_length(self):
        ...
        for signal_id in self.signal_order:
            ...
            if current_length < target_length:
                ...
                logger.debug(f"信号 {signal_id} 填充 {fill_count} 个值，目标长度: {target_length}")
    ...
    def get_time_range_data(self, signal_id, start_time, end_time):
        ...
        logger.debug(f"时间范围查询: {signal_id}, 原始点数={len(timestamps)}, 结果点数={len(result_times)}")
