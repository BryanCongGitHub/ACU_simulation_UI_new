...
from waveform_display import WaveformDisplay  # noqa: E402
from views.event_bus import ViewEventBus  # noqa: E402

from controllers.communication_controller import (  # noqa: E402
...
from model.device import Device, DeviceConfig  # noqa: E402
...
try:
    if not CONFIG_PATH.exists():
        ...
        with open(CONFIG_PATH, "w", encoding="utf-8") as cfg_file:
            json.dump(default_cfg, cfg_file, indent=2, ensure_ascii=False)
except Exception:
    pass
...
class ACUSimulator(QMainWindow):
    ...
    def _start_background_workers_if_needed(self):
        ...
        if not getattr(self, "parse_thread", None) or not self.parse_thread.isRunning():
            ...
            self._create_parse_worker()
        if (
            not getattr(self, "format_thread", None)
            or not self.format_thread.isRunning()
        ):
        ...
        try:
            if getattr(self, "parse_worker", None):
                QMetaObject.invokeMethod(
                    self.parse_worker, "stop", Qt.BlockingQueuedConnection
                )
    ...
    def _cleanup_format_worker(self):
        try:
            if getattr(self, "format_worker", None):
                QMetaObject.invokeMethod(
                    self.format_worker, "stop", Qt.BlockingQueuedConnection
                )
            if getattr(self, "format_thread", None):
                self.format_thread.quit()
    ...
    def setup_config_tab(self):
        ...
        _splitter = QSplitter(Qt.Vertical)
        config_layout.addWidget(_splitter)
        ...
        try:
            ...
            self.preview_btn.setIcon(
                _style.standardIcon(QStyle.SP_FileDialogContentsView)
            )
            self.save_config_btn.setIcon(
                _style.standardIcon(QStyle.SP_DialogSaveButton)
            )
            self.load_config_btn.setIcon(
                _style.standardIcon(QStyle.SP_DialogOpenButton)
            )
        except Exception:
            pass
        ...
        self.status_label.setStyleSheet(
            (
                "QLabel { "
                "background-color: #f8f8f8; padding: 8px; "
        ...
        self.life_signal_label.setStyleSheet(
            (
                "QLabel { "
                "background-color: #e8f4ff; padding: 5px; "
                "border: 1px solid #cce6ff; font-size: 9pt; "
                "}"
            )
        )
        ...
        self.receive_count_label.setStyleSheet(
            (
                "QLabel { "
                "background-color: #f7ffe8; padding: 5px; "
                "border: 1px solid #e6f6cc; font-size: 9pt; "
                "}"
            )
        )
        ...
        self.memory_status_label.setStyleSheet(
            (
                "QLabel { "
                "background-color: #fff7e8; padding: 5px; "
                "border: 1px solid #f3e6cc; font-size: 9pt; "
                "}"
            )
        )
...
• 设备响应数据长度: 64字节
    ...
    def setup_send_tab(self):
        ...
        for i, (name, byte_pos, bit_pos) in enumerate(bool_commands):
            ...
            checkbox.setChecked(
                self.control_values["bool_commands"].get((byte_pos, bit_pos), False)
            )
            checkbox.stateChanged.connect(
                lambda state, bp=byte_pos, bit=bit_pos: self.on_bool_command_changed(
                    bp, bit, state
                )
            )
        ...
        for name, byte_pos in freq_commands:
            ...
            spinbox.valueChanged.connect(
                lambda value, bp=byte_pos: self.on_freq_control_changed(bp, value)
            )
        ...
        for i, device in enumerate(devices):
            ...
            checkbox.stateChanged.connect(
                lambda state, idx=i: self.on_isolation_command_changed(idx, state)
            )
            checkbox.clicked.connect(self.on_control_changed)
        ...
        for i, device in enumerate(devices):
            ...
            checkbox.stateChanged.connect(
                lambda state, idx=i: self.on_start_command_changed(idx, state)
            )
            checkbox.clicked.connect(self.on_control_changed)
        ...
        for name, byte_pos, bit_pos in chu_commands:
            ...
            checkbox.setChecked(
                self.control_values["chu_controls"].get((byte_pos, bit_pos), False)
            )
            checkbox.stateChanged.connect(
                lambda state, bp=byte_pos, bit=bit_pos: self.on_chu_control_changed(
                    bp, bit, state
                )
            )
            checkbox.clicked.connect(self.on_control_changed)
        ...
        redundant_commands = [("INV1冗余启动", 67, 0), ("INV4冗余启动", 67, 1)]
        ...
        for name, byte_pos, bit_pos in redundant_commands:
            ...
            current_value = self.control_values["redundant_commands"].get(
                (byte_pos, bit_pos), False
            )
            checkbox.setChecked(current_value)

            def _handle_state(state, bp=byte_pos, bit=bit_pos):
                self.on_redundant_command_changed(bp, bit, state)

            checkbox.stateChanged.connect(_handle_state)
            checkbox.clicked.connect(self.on_control_changed)
        ...
        for i, device in enumerate(inv_devices):
            ...
            seconds = self.control_values["start_times"].get(start_time_positions[i], 0)
            spinbox.setValue(seconds)
            spinbox.setMaximumWidth(80)
            self.control_values["start_times"][
                start_time_positions[i]
            ] = spinbox.value()
            spinbox.valueChanged.connect(
                lambda value, pos=start_time_positions[i]: self.on_start_time_changed(
                    pos, value
                )
            )
            spinbox.valueChanged.connect(self.on_control_changed)
        ...
        self.branch3_voltage_spin.setValue(
            self.control_values["branch_voltages"].get(154, 0)
        )
        ...
        self.branch4_voltage_spin.setValue(
            self.control_values["branch_voltages"].get(156, 0)
        )
        ...
        self.raw_data_edit.setPlaceholderText(
            "可在此输入十六进制数据覆盖自动生成的数据"
        )
    ...
    def setup_parse_tab(self):
        ...
        self.parse_history_table.setHorizontalHeaderLabels(
            ["时间", "来源", "设备类型", "数据长度", "解析状态"]
        )
        self.parse_history_table.setSelectionBehavior(QTableWidget.SelectRows)
        self.parse_history_table.doubleClicked.connect(
            self.on_parse_history_double_clicked
        )
    ...
    def update_data_preview(self):
        ...
        try:
            ...
            lines = [
                "=== ACU发送数据预览 (完整协议) ===",
                f"数据长度: {len(self.send_data_buffer)} 字节",
                f"生命信号: {self.ccu_life_signal}",
            ...
            for title, entries in bool_sections:
                lines.append(title)
                for label, key in entries:
                    state = (
                        "开启" if controls["bool_commands"].get(key, False) else "关闭"
                    )
            ...
            for label, key in freq_entries:
                freq_value = controls["freq_controls"].get(key, 0)
                lines.append(f"• {label}: {freq_value} Hz")
            lines.append("")

            lines.append("隔离指令 (字节64):")
            ...
            for i, device in enumerate(devices):
                iso_state = (
                    "隔离"
                    if self.control_values["isolation_commands"].get(i, False)
                    else "正常"
                )
                lines.append(f"• {device}: {iso_state}")

            lines.append("")
            lines.append("启动指令 (字节65):")
            for i, device in enumerate(devices):
                start_state = (
                    "启动"
                    if self.control_values["start_commands"].get(i, False)
                    else "停止"
                )
                lines.append(f"• {device}: {start_state}")

            lines.append("")
            lines.append("CHU控制 (字节66):")
            chu_entries = [
                ("CHU3启动", (66, 0)),
                ("CHU4启动", (66, 1)),
                ("CHU3隔离", (66, 2)),
            ...
            for label, key in chu_entries:
                chu_state = (
                    "开启"
                    if self.control_values["chu_controls"].get(key, False)
                    else "关闭"
                )
                lines.append(f"• {label}: {chu_state}")

            lines.append("")
            lines.append("冗余启动 (字节67):")
            redundant_entries = [
                ("INV1冗余启动", (67, 0)),
                ("INV4冗余启动", (67, 1)),
            ]
            for label, key in redundant_entries:
                redundant_state = (
                    "开启"
                    if self.control_values["redundant_commands"].get(key, False)
                    else "关闭"
                )
                lines.append(f"• {label}: {redundant_state}")
            ...
            start_time_positions = [142, 144, 146, 148, 150, 152]
            for i, device in enumerate(inv_devices):
                start_time_key = start_time_positions[i]
                start_value = controls["start_times"].get(start_time_key, 0)
                lines.append(f"• {device}: {start_value} 秒")

            lines.append("")
            lines.append("电压和温度配置:")
            lines.append(f"• 3支路电压: {controls['branch_voltages'].get(154, 0)} V")
            lines.append(f"• 4支路电压: {controls['branch_voltages'].get(156, 0)} V")
            lines.append(f"• 蓄电池温度: {controls['battery_temp']} °C")

            lines.append("")
            lines.append("=== 关键字节状态 ===")
            lines.append(
                "字节8 (基本控制):  "
                f"{self.send_data_buffer[8]:08b} = {self.send_data_buffer[8]:02X}h"
            )
            lines.append(
                "字节9 (设备控制):  "
                f"{self.send_data_buffer[9]:08b} = {self.send_data_buffer[9]:02X}h"
            )
            lines.append(
                "字节64(隔离指令): "
                f"{self.send_data_buffer[64]:08b} = {self.send_data_buffer[64]:02X}h"
            )
            lines.append(
                "字节65(启动指令): "
                f"{self.send_data_buffer[65]:08b} = {self.send_data_buffer[65]:02X}h"
            )
            lines.append(
                "字节66(CHU控制):  "
                f"{self.send_data_buffer[66]:08b} = {self.send_data_buffer[66]:02X}h"
            )
            lines.append(
                "字节67(冗余启动): "
                f"{self.send_data_buffer[67]:08b} = {self.send_data_buffer[67]:02X}h"
            )

            lines.append("")
            lines.append("=== 十六进制数据 (前100字节) ===")
            ...
            for i in range(0, display_length, 16):
                line_hex = " ".join(
                    f"{self.send_data_buffer[j]:02X}"
                    for j in range(i, min(i + 16, display_length))
                )
            ...
            if len(self.send_data_buffer) > display_length:
                remaining = len(self.send_data_buffer) - display_length
                lines.append(f"... (剩余 {remaining} 字节)")
    ...
    def send_periodic_data(self):
        ...
        if self.is_sending:
            ...
            logger.debug(
                f"[发送周期] 时间戳: {current_time:.3f}, 生命信号: {self.ccu_life_signal}"
            )

            # 传递副本避免数据竞争，并确保是bytearray类型
            self.view_bus.waveform_send.emit(
                bytearray(self.send_data_buffer), current_time
            )
    ...
    def display_parsed_data(self, parsed_data, device_type):
        ...
        root_item = QTreeWidgetItem(
            self.parse_tree, [f"设备类型: {device_type}", "", ""]
        )
        ...
        try:
            ...
            if self.parse_table_buffer:
                ...
                while self.parse_table_buffer and processed_parse < MAX_PARSE_PER_FLUSH:
                    ...
                    self.parse_history_table.setItem(
                        row, 0, QTableWidgetItem(rec.get("timestamp", ""))
                    )
                    self.parse_history_table.setItem(
                        row, 1, QTableWidgetItem(rec.get("address", ""))
                    )
                    self.parse_history_table.setItem(
                        row, 2, QTableWidgetItem(rec.get("device_type", ""))
                    )
                    self.parse_history_table.setItem(
                        row, 3, QTableWidgetItem(str(rec.get("data_length", 0)))
                    )
                    self.parse_history_table.setItem(
                        row, 4, QTableWidgetItem("解析成功")
                    )
                    processed_parse += 1
                ...
                logger.debug(
                    "[perf] parse_table_buffer flushed: "
                    f"requested={count}, processed={processed_parse}, "
                    f"elapsed_ms={elapsed:.1f}"
                )

            # 如果有最近解析结果需要显示，显示并清理
            if (
                hasattr(self, "last_parsed_for_display")
                and self.last_parsed_for_display
            ):
    ...
    def on_parse_history_double_clicked(self, index):
        ...
        if row < len(self.parsed_data_history):
            ...
            self.display_parsed_data(
                parse_info["parsed_data"], parse_info["device_type"]
            )
    ...
    def load_config(self):
        ...
        try:
            ...
            self.acu_send_port_edit.setText(
                str(cfg.get("acu_send_port", self.acu_send_port_edit.text()))
            )
            self.acu_receive_port_edit.setText(
                str(cfg.get("acu_receive_port", self.acu_receive_port_edit.text()))
            )
            self.target_ip_edit.setText(
                cfg.get("target_ip", self.target_ip_edit.text())
            )
            self.target_receive_port_edit.setText(
                str(
                    cfg.get("target_receive_port", self.target_receive_port_edit.text())
                )
            )
            self.period_spin.setValue(
                int(cfg.get("period_ms", self.period_spin.value()))
            )
    ...
    def on_error_occurred(self, error_msg):
        ...
        self.status_label.setStyleSheet(
            (
                "QLabel { "
                "background-color: #ffe0e0; padding: 8px; "
    ...
    def on_status_updated(self, status_msg):
        ...
        if "错误" in status_msg:
            ...
            self.status_label.setStyleSheet(
                (
                    "QLabel { "
                    "background-color: #e0f0ff; padding: 8px; "
            ...
            self.status_label.setStyleSheet(
                (
                    "QLabel { "
                    "background-color: #e0ffe0; padding: 8px; "
    ...
    def check_memory_usage(self):
        ...
        if memory_mb > 500:
            self.memory_status_label.setStyleSheet(
                (
                    "QLabel { "
                    "background-color: #ffcccc; padding: 5px; "
                    "border: 1px solid #ff0000; "
                    "}"
                )
            )
            ...
            self.memory_status_label.setStyleSheet(
                (
                    "QLabel { "
                    "background-color: #fff0cc; padding: 5px; "
            ...
            self.memory_status_label.setStyleSheet(
                (
                    "QLabel { "
                    "background-color: #f0ffe0; padding: 5px; "
                    "border: 1px solid #00cc00; "
                    "}"
                )
            )
